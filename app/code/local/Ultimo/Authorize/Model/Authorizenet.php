<?php
class Ultimo_Authorize_Model_Authorizenet extends Mage_Payment_Model_Method_Cc
{
    /*
     * AIM gateway url
     */
    ...
    const METHOD_CODE = 'ultimoauthorize';
     ...
     * Form block type
     */
    protected $_formBlockType = 'ultimoauthorize/authorizenet_form_cc';

    /**
     * Info block type
     */
    protected $_infoBlockType = 'ultimoauthorize/authorizenet_info_cc';
    ...
    protected $_partialAuthorizationChecksumDataKeys = array(
        'x_cpversion', 'x_test_request', 'x_login', 'x_market_type', 'x_device_type', 'x_test_request', 'x_allow_partial_auth', 'x_amount',
    ...
    /**
     * DUNGNT
     * Fields for creating place request checksum for CNP
     *
     * @var array
     */
    protected $_partialAuthorizationChecksumDataKeysCNP = array(
     ...
     */
    //+------------ WEBPROVISE UPDATE - REPLACE REFUND FUNCTION  - 11/09/2013
    ...
    {
        if($this->getConfigData('card_present')){
            ...
            $cclast4 = $additional[$keys[0]]['cc_last4'];
            //Mage::log( 'Payment Info: '.$cclast4 );

            if ($transactionId && $amount > 0) {
                $orderTime = strtotime($order->getCreatedAt());
                $currentTime = time();
                $timeDiff = abs($currentTime - $orderTime) / 3600;
                //Mage::log( 'Order Time: '.$orderTime );
                //Mage::log( 'Current Time: '.$currentTime );
                //Mage::log( 'Hour Difference: '.$timeDiff );
                if ($timeDiff  > 12) {
                ...
                $request->setXRefTransId($transactionId);
                //Mage::log( 'Trans ID: '.$transactionId );
                /**
                 * need to send last 4 digit credit card number to authorize.net
                 * otherwise it will give an error
                 */
                $request->setXCardNum($cclast4);
                //Mage::log( 'CC: '.$cclast4 );

                $result = $this->_postRequest($request);
                if ($result->getResponseCode()==self::RESPONSE_CODE_APPROVED) {
                    //Mage::log('RESPONSE: '.$result->getTransactionId());
                    $comment = 'Authorize.Net Refund Approved ('.$tranType.' - $'.$amount.') - Transaction ID: '.$result->getTransactionId();
            ...
            case self::RESPONSE_CODE_HELD:
                ...
                if ($result->getResponseReasonCode() == self::RESPONSE_REASON_CODE_PARTIAL_APPROVE) {
                    if($this->getConfigData('card_present')){
                        ...
                        $checksum = $this->_generateChecksum($request, $this->_partialAuthorizationChecksumDataKeysCNP);
                    }
    ...
    protected function _partialAuthorization($payment, $amount, $requestType)
    {
        ...
        if ($this->getConfigData('partial_authorization_checksum_checking')) {
            ...
            if($this->getConfigData('card_present')){
                ...
                $newChecksum = $this->_generateChecksum($firstPlacingRequest, $this->_partialAuthorizationChecksumDataKeysCNP);
            }
...
//        if($warehouse){
//            $transkey = $warehouse->getTransactionKey();
//            $loginid = $warehouse->getAuthorizeId();
//        }
//        else{
        $transkey = $this->getConfigData('trans_key');
        $loginid = $this->getConfigData('login');
//        }
        if($this->getConfigData('card_present')){
    ...
    protected function _postRequest(Varien_Object $request)
    {
        ...
        if ($r) {
            if($this->getConfigData('card_present')){
                $result->setResponseCode((int)str_replace('"','',$r[1]))
                    ->setResponseReasonCode((int)str_replace('"','',$r[2]))
                    ->setResponseReasonText($r[3])
                    ->setApprovalCode($r[4])
                    ->setAvsResultCode($r[5])
                    ->setCardCodeResponseCode($r[6])
                    ->setTransactionId($r[7])
                    ->setMd5Hash($r[8])
                ;
            }
            else {
...
//    {
//        $id = Mage::getSingleton('admin/session')->getWarehouseId();
//        $warehouse = Mage::getModel('xwarehouse/warehouse')->load($id);
//        return $warehouse;
//    }
    ...
    public function isAvailable($quote = null)
      {
          ...
          $availableInXPOSonly = Mage::getStoreConfig('payment/ultimoauthorize/allow_in_xpos_only');

          if((boolean) $availableInXPOSonly && Mage::app()->getFrontController()->getRequest()->getControllerModule() != 'SM_XPos_Adminhtml') {
               return false;
          }
