...
//            $this->_getOrderCreateModel()->setRecollect(true);
            ...
            $paymentData = $this->getRequest()->getPost('payment');
            if ($paymentData && isset($paymentData['method']) && (in_array($paymentData['method'], array('ccsave', 'authorizenet', 'ultimoauthorize','verisign')))) {
                $this->_getOrderCreateModel()->setPaymentData($paymentData);
            }
            if (isset($data['shipping_method'])) {
    ...
    public function createInvoiceAction()
    {
        ...
        if ($dataInvoice['createInvoice'] == '1') {
            ...
            try {
                ...
                if($order->getPayment()->getMethod() == 'ultimoauthorize') {
                    $amountToCapture = Mage::app()->getRequest()->getParam('captureAmount');
                    $invoice->setTotalPaid($amountToCapture);
                    $invoice->setBaseTotalPaid($amountToCapture);
//                $invoice->setGrandTotal($amountToCapture);
//                $invoice->setBaseGrandTotal($amountToCapture);
                } else {
            ...
            if($order->getPayment()->getMethod() == 'ultimoauthorize') {
                ...
                $order->setBaseTotalRefunded((float) $order->getBaseGrandTotal() - (float) $amountToCapture);
                $order->setTotalRefunded((float) $order->getGrandTotal() - (float) $amountToCapture);
                $order->setBaseSubtotalRefunded((float) $order->getBaseSubtotalRefunded() - (float) $amountToCapture);
                $order->setSubtotalRefunded((float) $order->getGrandTotal() - (float) $amountToCapture);
                $order->setTotalOfflineRefunded(0);
                $order->setTotalOnlineRefunded((float) $order->getGrandTotal() - (float) $amountToCapture);
