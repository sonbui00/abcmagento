 ...
 * Time: 11:46 AM
 */
class SM_XReport_Block_Adminhtml_Dashboard_Chart extends Mage_Adminhtml_Block_Dashboard_Graph {
    ...
    public function setDataRow($type = 'revenue') {
        if ($type == 'revenue') {
            $this->_dataRows = array('revenue');
    ...
    public function getDataHelper() {
        $ssStoreId = Mage::getSingleton('xreport/session')->getData('store_id');
        if ($ssStoreId != 0)
            $this->helper($this->getDataHelperName())->setParam('store', $ssStoreId);
        return $this->helper($this->getDataHelperName());
    }

    public function getDataSalesChart($period, $filter = 0, $getNgChart = true) {
//        return true;
        $this->getDataHelper()->setParam('period', $period);

        /*DO: Create collection*/
        if ($filter == 0)
            $this->getDataHelper()->getCollection();
        ...
        if (sizeof($this->_axisLabels) > 0) {
            ...
            foreach ($this->_axisLabels as $idx => $labels) {
                if ($idx == 'x') {
                    ...
                    $dataNgChart = $this->initDataForNgChart($this->_axisLabels[$idx], $this->_allSeries[$this->_dataRows[0]], $period);
                    if ($getNgChart)
                        return $dataNgChart;
    ...
    private function initDataForNgChart($axisLabels, $allSeries, $period = '24h') {
        $dataLable = array();
        $dataValue = array();
